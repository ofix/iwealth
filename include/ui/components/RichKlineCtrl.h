...
class RichKlineCtrl:public wxControl {
    ...
    public:
        ...
        RichKlineCtrl(wxWindow* parent,wxWindowID id,
    ...
    protected:
        ...
        std::vector<KlineItem> GetWeekKlines();
        std::vector<KlineItem> GetMonthKlines();
        float GetRectMinPrice(std::vector<KlineItem>& data,int begin, int end);
        float GetRectMaxPrice(std::vector<KlineItem>& data,int begin, int end);
        uiKlineRange  GetKlineRangeZoomIn(long totalKLines, long widthContainer,
                                          int16_t klineWidth=5,int16_t klineSpan=2,
                                          long crossLine = NO_CROSS_LINE);//放大K线图
        uiKlineRange  GetKlineRangeZoomOut(long totalKLines,long crossLine = NO_CROSS_LINE);//以十字线为中心，否则右侧缩放
    ...
    protected:
        ...
        std::vector<KlineItem> m_klines; // the k line data
        std::vector<KlineItem> m_weekKlines; //week k line data;
        std::vector<KlineItem> m_monthKlines; //month k line data;
