...
std::string ChinesePinYin::GetFirstLetters(const std::string& chinese) {
    ...
    // 395个字符串，每个字符串长度不超过6
    const static char pinyin_dict[396][7] = {
        ...
        "hua",   "huai",  "huan",  "huang",  "hui",   "hun",   "huo",    "ji",    "jia",
        "jian",  "jiang", "jiao",  "jie",    "jin",   "jing",  "jiong",  "jiu",   "ju",
        ...
        "mou",   "mu",    "na",    "nai",    "nan",   "nang",  "nao",    "ne",    "nei",
        ...
        "qian",  "qiang", "qiao",  "qie",    "qin",   "qing",  "qiong",  "qiu",   "qu",
        "quan",  "que",   "qun",   "ran",    "rang",  "rao",   "re",     "ren",   "reng",
        "ri",    "rong",  "rou",   "ru",     "ruan",  "rui",   "run",    "ruo",   "sa",
    ...
    try {
        ...
        for (size_t j = 0, unicode_point = 0; j < length;) {
            ...
            if (chinese.at(j) >= 0 && chinese.at(j) < 128) {
                pin_yin += chinese.at(j);
                j++;
                continue;
            }
            // 汉字处理
            unicode_point = chinese.at(j) * 256 + chinese.at(j + 1) + 256;
            if (unicode_point > 0 && unicode_point < 160) {  // 非汉字
                pin_yin += chinese.at(j);
                j++;
            } else {
                // 汉字
                for (size_t i = (sizeof(unicode_dict) / sizeof(unicode_dict[0]) - 1);
                     i >= 0; --i) {
                    // 查找字典
                    if (unicode_dict[i] <= unicode_point) {
                        pin_yin += pinyin_dict[i];
                        break;
                    }
                }
                j += 2;
