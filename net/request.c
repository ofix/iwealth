...
yyjson_doc *doGet(conn_t *conn)
{
    ...
    yyjson_doc *response = _doRequest(conn);
    _curlClose(conn);
    return response;
}

// 发送 HTTPS POST 请求
yyjson_doc *doPost(conn_t *conn)
{
    _curlInit(conn);
    if (conn->upload_info && conn->upload_info->upload)
    {
        ...
        curl_easy_setopt(conn->easy_conn->easy_curl, CURLOPT_READDATA, conn->upload_info->file_src);
...
// 发送 HTTPS PATCH 请求
yyjson_doc *doPatch(conn_t *conn)
{
    _curlInit(conn);
...
// 发送 HTTPS DELETE 请求
yyjson_doc *doDelete(conn_t *conn)
{
    _curlInit(conn);
...
// 内部请求调用
yyjson_doc *_doRequest(conn_t *conn)
{
    ...
    // 设置HTTPS请求响应码
    curl_easy_getinfo(conn->easy_conn->easy_curl, CURLINFO_RESPONSE_CODE, &conn->http_code);
    ...
    yyjson_doc *response = yyjson_read(conn->recive_buff, strlen(conn->recive_buff), 0);
    if (debug)
    {
        printf(
            "\n===============================\n%s\n============================"
...
void _curlSetCommonOptions(conn_t *conn)
{
    ...
    curl_easy_setopt(conn->easy_conn->easy_curl, CURLOPT_WRITEFUNCTION, _curlOnResponseBodyRecv);
    curl_easy_setopt(conn->easy_conn->easy_curl, CURLOPT_WRITEDATA, conn);
    curl_easy_setopt(conn->easy_conn->easy_curl, CURLOPT_HEADERFUNCTION, _curlOnResponseHeaderRecv);
...
size_t _curlOnResponseBodyRecv(void *ptr, size_t size, size_t nmemb, void *data)
{
...
size_t _curlOnResponseHeaderRecv(void *ptr, size_t size, size_t nmemb, void *data)
{
